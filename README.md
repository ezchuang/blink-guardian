...
- 角度 profile 使用分界緩衝區，微小座標抖動不會觸發切換；候選角度須持續 160ms 才會切換，等待期間暫停判定但不清除正在進行的閉眼狀態。明確換角度仍會校正。
- 主頁與迷你窗平常顯示「監測中」；閉眼休息、校正、影像中斷等狀態持續 1 秒才切換文字與狀態燈，避免短暫眨眼造成閃爍。開始、停止與錯誤訊息立即顯示；偵測、計時與真正提醒不受此顯示延遲影響。
