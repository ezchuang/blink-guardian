...
- 眨眼提醒預設開啟、距離提醒預設關閉，可獨立切換。關閉眨眼後停止眨眼分析、計數、節奏與遠望提醒；兩項都關閉會停止監測、釋放鏡頭及模型。全部關閉後重新啟用功能，需按「開始監測」。
- 選項與距離基準只留在本次頁面記憶體，不寫入裝置儲存。迷你窗首次接收主頁的功能選擇，重新整理則回到預設；可透過迷你窗的「偵測設定」修改。
