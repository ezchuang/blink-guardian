...
- 眨眼提醒預設開啟，距離、坐姿／駝背提醒預設關閉，可獨立切換。關閉眨眼後停止眨眼分析、計數、節奏與遠望提醒；全部關閉會停止監測、釋放鏡頭及模型。全部關閉後重新啟用功能，需按「開始監測」。
- 選項與距離、坐姿基準只留在本次頁面記憶體，不寫入裝置儲存。迷你窗首次接收主頁的功能選擇，重新整理則回到預設。
- 主頁保留小型鏡頭預覽、統計與主要操作；完整設定從「偵測與提醒設定」開啟，迷你窗則用「偵測設定」。監測啟動後收起介紹區，桌面主要控制以一頁內可操作為目標；小螢幕與放大字體時允許捲動，不裁切內容。
- 開啟距離提醒並開始監測後，先面向鏡頭坐在合適的位置，按「以目前坐姿設定基準」。收集至少 2 秒的穩定資料後開始比較，基準不會自動適應。停止重開或重新開啟距離功能時需重設；移動鏡頭後也應手動重設。
- 距離以臉部影像尺寸相對基準的比例估計，不是公分、醫療安全距離或駝背判定。相對尺寸放大至少 25% 且持續 5 秒才提醒；降至 15% 以內才解除靠近狀態，同一段持續靠近只提醒一次，前後距離提醒至少間隔 60 秒。轉頭、明顯角度差異、臉部出框或失去追蹤時暫停判定。
- 距離分析重用 Face Landmarker，不載入額外模型，每秒檢查 2 次。只開距離時，模型推論也降至 2 FPS 並停用 blendshape 輸出；相機仍持續擷取影像，實際耗電需依裝置測量。
- 眨眼使用兩短聲、距離使用一聲較長低音，可各別試聽；提示音總開關控制全部聲音。不同提醒不重疊，提醒視窗間至少間隔 8 秒。距離提醒也會記錄在歷史事件中，閉眼不會清除它。
- 坐姿提醒需另行按「設定自然坐姿基準」，讓頭部與雙肩完整入鏡並維持自然坐姿至少 3 秒。使用雙耳中點到肩膀中點的垂直距離除以肩寬，與手動基準比較；降低至少 0.15 個肩寬且持續 10 秒才提醒，回到 0.08 以內解除。同一段偏離只提醒一次，坐姿提醒間隔至少 60 秒。這些是初版工程閾值，尚未做真人準確度驗證。
- 坐姿功能使用 Pose Landmarker Lite，在 Web Worker 中以 CPU 推論，最多每秒 2 次、一次僅處理一幀，擷取寬度 320px；僅啟用並開始監測後才載入，關閉或停止即終止 worker。只有坐姿功能開啟時不執行 Face Landmarker。背景頁面不送入新影格，舊資料不能累積提醒時間。
- 頭肩遮擋、低信心、明顯轉頭或側傾時不判定；頭肩相對高度降低也可能來自低頭或其他動作，不代表脊椎曲度或醫療上的駝背。鏡頭移動、坐姿基準不合適時應重新設定；停止重開亦需重設。
- 坐姿提醒使用高低兩聲，與眨眼、距離音效不同，並記錄於歷史事件中。新增模型仍會增加 CPU 與耗電，低頻及背景執行緒不能保證所有裝置都順暢。
...
- 第一次使用需從 CDN 下載 MediaPipe JavaScript、WebAssembly 與啟用功能所需的 Face Landmarker／Pose Landmarker Lite model。
...
- MediaPipe Face Landmarker、選配 Pose Landmarker Lite（Web Worker）
